...
MASTER_PASSWORD = "God Is All"  # For admin dashboard login
...
def init_db():
    ...
    # Migration: Add ban columns if they don't exist (for existing DBs)
    try:
        ...
        print("Migrated: Added ban columns to users table")
...
def check_user_banned():
    ...
    if 'user_id' in session:
        # Skip check for logout route (allow banned users to logout)
        if request.endpoint in ['logout', 'check_ban', 'static']:
            return None
...
def callback():
    ...
    # Check if banned
    is_banned, reason, _ = check_ban_status(user['id'])
    if is_banned:
        return render_template_string("""
        <h1>Account Banned</h1>
        <p>Reason: {{ reason }}</p>
...
def check_ban():
    """API endpoint for frontend to check ban status"""
    if 'user_id' not in session:
...
def admin_ban_user():
    ...
    # Check role hierarchy - prevent banning owners/admins unless you're owner
    ...
    # duration == 'permanent' leaves expires_at as None
    ...
    return jsonify({
        'success': True,
        'message': f"User {target['name']} has been banned",
        'expires_at': expires_at
    })
...
if __name__ == '__main__':
    ...
    if os.environ.get('RENDER') != 'true':
        print("\n" + "="*70)
        print("BIBLE AI - LOCAL MODE (with Ban System)")
        print("="*70)
